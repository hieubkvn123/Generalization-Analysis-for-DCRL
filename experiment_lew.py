...
def train_one_epoch_spectral_decay(
        ...
        min_sigma: float = 0.9):
    """
    Training with spectral decay regularisation.

    Adds a penalty   lambda * sum_ell  sigma_1(W_ell)^2
    to the cross-entropy loss, which pushes singular values down.

    After each gradient step we hard-clip the spectral norm from below
    so that it stays >= min_sigma (preventing collapse to zero).

    Args:
        decay_lambda : strength of the spectral-decay penalty.
        min_sigma    : floor on the spectral norm of every weight matrix
                       (enforced by a post-step projection / rescale).
    """
    model.train()
    total_loss, correct, total = 0., 0, 0
    ...
    for imgs, labels in loader:
        ...
        # --- spectral-decay penalty (differentiable via power-iteration) ---
        spec_penalty = torch.tensor(0.0, device=device)
        for name, param in model.named_parameters():
            if 'weight' in name and param.dim() >= 2:
                W2 = param.view(param.shape[0], -1)
                # power-iteration estimate of sigma_1 (keeps computation graph)
                with torch.no_grad():
                    u = torch.randn(W2.shape[0], device=device, dtype=W2.dtype)
                    u = u / u.norm()
                    for _ in range(5):
                        v = W2.t().mv(u)
                        v = v / v.norm()
                        u = W2.mv(v)
                        u = u / u.norm()
                # one more step with grad enabled to get differentiable sigma
                v_sg = v.detach()
                sigma = W2.mv(v_sg).norm()          # differentiable w.r.t. param
                spec_penalty = spec_penalty + sigma ** 2

        loss = ce_loss + decay_lambda * spec_penalty
        loss.backward()
        optimizer.step()

        # --- floor: ensure spectral norm >= min_sigma ---
        with torch.no_grad():
            for name, param in model.named_parameters():
                if 'weight' in name and param.dim() >= 2:
                    sigma = spectral_norm_matrix(param.data)
                    if sigma < min_sigma:
                        param.data.mul_(min_sigma / sigma.clamp(min=1e-8))

        total_loss += ce_loss.item() * imgs.size(0)
        correct += out.argmax(1).eq(labels).sum().item()
        total += imgs.size(0)
...
def train_one_epoch_approx_sparse(
        model, loader, optimizer, device,
        sparse_lambda: float = 1e-3,
        sparse_alpha: float = 0.1):
    """
    Training with approximate-sparsity regularisation.

    Adds a smooth surrogate for the L0 "near-zero" count:

        penalty = lambda * sum_ell  sum_{i,j}  w_{ij}^2 / (w_{ij}^2 + alpha^2)

    This is the "continuous relaxation of L0" (MCP / Cauchy surrogate).
    When |w_{ij}| >> alpha the term approaches 1 (counted as non-zero).
    When |w_{ij}| << alpha the term approaches 0.
    Minimising it pushes weights toward |w| = 0 without snapping them exactly
    to zero, producing the desired near-zero but not exactly-zero entries.

    Args:
        sparse_lambda : strength of the sparsity penalty.
        sparse_alpha  : scale threshold; weights with |w| << alpha are driven
                        toward zero.  Typical value: 0.05 – 0.2.
    """
    model.train()
    total_loss, correct, total = 0., 0, 0
    alpha2 = sparse_alpha ** 2
    ...
    for imgs, labels in loader:
        ...
        # --- approximate sparsity penalty (smooth L0 surrogate) ---
        sparse_penalty = torch.tensor(0.0, device=device)
        for name, param in model.named_parameters():
            if 'weight' in name and param.dim() >= 2:
                w2 = param.pow(2)
                # Cauchy / continuous-relaxation-of-L0 surrogate
                sparse_penalty = sparse_penalty + (w2 / (w2 + alpha2)).sum()

        loss = ce_loss + sparse_lambda * sparse_penalty
        loss.backward()
        optimizer.step()
...
def train_one_epoch_mcp_sparse(
        model, loader, optimizer, device,
        sparse_lambda: float = 1e-3,
        sparse_alpha: float = 0.1,
        mcp_gamma: float = 3.0):
    """
    Training with MCP (Minimax Concave Penalty) sparsity regularisation.

    MCP is a non-convex penalty that is:
      - Linear (L1-like) for small |w|, creating a strong pull toward zero.
      - Exactly zero for |w| > gamma * alpha, so large weights are completely
        unpenalised (unlike L1/Cauchy which always shrink large weights).

    Per-entry penalty:
        phi_MCP(w) = |w| - w^2 / (2 * gamma * alpha)   if |w| <= gamma * alpha
                   = (gamma * alpha) / 2                 if |w| >  gamma * alpha

    This is approximated in a smooth, fully differentiable form as:

        phi_MCP(w) = alpha * gamma * (1 - (1 - min(|w|, gamma*alpha) / (gamma*alpha))^2) / 2

    which we implement directly via clamping:

        phi_MCP(w) = |w| - clamp(w^2, max=(gamma*alpha)^2) / (2 * gamma * alpha)

    Gradient w.r.t. w:
        d phi / d w = sign(w) * max(0,  1 - |w| / (gamma * alpha))

    i.e. gradient is sign(w) for tiny |w|, and decays linearly to zero at
    |w| = gamma * alpha. Beyond that: zero gradient — large weights are frozen
    by the penalty, so the CE loss alone controls them.

    Args:
        sparse_lambda : overall penalty weight.
        sparse_alpha  : base scale (same role as in Cauchy; sets the "small" region).
        mcp_gamma     : controls the transition point gamma * alpha beyond which
                        the penalty saturates.  Larger gamma = softer truncation.
                        Typical range: 2 – 5.
    """
    model.train()
    total_loss, correct, total = 0., 0, 0
    threshold = mcp_gamma * sparse_alpha          # |w| beyond which grad = 0
    ...
    for imgs, labels in loader:
        ...
        # --- MCP penalty (per-layer, normalised by number of parameters) ---
        mcp_penalty = torch.tensor(0.0, device=device)
        for name, param in model.named_parameters():
            if 'weight' in name and param.dim() >= 2:
                abs_w = param.abs()
                # phi_MCP(w) = |w| - clamp(w^2, max=threshold^2) / (2 * threshold)
                phi = abs_w - param.pow(2).clamp(max=threshold ** 2) / (2.0 * threshold)
                # normalise by parameter count so lambda is width-invariant
                mcp_penalty = mcp_penalty + phi.sum() / param.numel()

        loss = ce_loss + sparse_lambda * mcp_penalty
        loss.backward()
        optimizer.step()
        project_spectral_norm_(model, max_sigma=1.2)
...
def train_one_epoch_annealed_sparse(
        model, loader, optimizer, device,
        sparse_lambda: float = 1e-3,
        sparse_alpha: float = 0.1,
        epoch: int = 1,
    ...
    """
    Training with Cauchy sparsity penalty and an annealed (upward) lambda schedule.

    The effective penalty weight lambda(t) follows three phases:

        Phase 1 — Warmup  [0, warmup_fraction * T):
            lambda(t) = 0   (pure CE; network finds a good basin first)

        Phase 2 — Linear ramp  [warmup_fraction * T, (warmup_fraction + anneal_fraction) * T):
            lambda(t) = sparse_lambda * (t - t_warm) / T_anneal
            (penalty is introduced gradually so CE is not overwhelmed)

        Phase 3 — Full penalty  [(warmup_fraction + anneal_fraction) * T, T]:
            lambda(t) = sparse_lambda

    where T = total_epochs, t_warm = warmup_fraction * T,
    T_anneal = anneal_fraction * T.

    The per-entry penalty is identical to train_one_epoch_approx_sparse
    (Cauchy surrogate), normalised per layer for width-invariance.

    Args:
        sparse_lambda     : target (maximum) penalty weight.
        sparse_alpha      : near-zero threshold for the Cauchy surrogate.
        epoch             : current epoch (1-indexed).
        total_epochs      : total number of training epochs.
        warmup_fraction   : fraction of epochs with lambda=0 (default: 0.2).
        anneal_fraction   : fraction of epochs over which lambda ramps up
                            from 0 to sparse_lambda (default: 0.5).
    """
    model.train()
    total_loss, correct, total = 0., 0, 0
    alpha2 = sparse_alpha ** 2

    # --- compute effective lambda for this epoch ---
    t_warm   = warmup_fraction * total_epochs
    t_anneal = anneal_fraction * total_epochs
    if epoch <= t_warm:
        eff_lambda = 0.0                                         # phase 1: warmup
    elif epoch <= t_warm + t_anneal:
        eff_lambda = sparse_lambda * (epoch - t_warm) / t_anneal  # phase 2: ramp
    else:
        eff_lambda = sparse_lambda                               # phase 3: full
    ...
    for imgs, labels in loader:
        ...
        if eff_lambda > 0.0:
            # --- Cauchy surrogate, normalised per layer ---
            sparse_penalty = torch.tensor(0.0, device=device)
            for name, param in model.named_parameters():
                if 'weight' in name and param.dim() >= 2:
                    w2 = param.pow(2)
                    sparse_penalty = sparse_penalty + (w2 / (w2 + alpha2)).sum() / param.numel()
            loss = ce_loss + eff_lambda * sparse_penalty
        else:
            loss = ce_loss                                        # warmup: no penalty
    ...
    return total_loss / total, correct / total, eff_lambda
...
def run_experiment(args):
    ...
    for fc_width in args.widths:
        ...
        regime = getattr(args, 'regime', 'standard')
        ckpt_path = os.path.join(args.save_dir, f'model_w{fc_width}_{regime}.pt')
        ...
        if os.path.exists(ckpt_path) and not args.retrain:
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                elif regime == 'mcp_sparse':
                    tr_loss, tr_acc = train_one_epoch_mcp_sparse(
                        model, train_loader, optimizer, device,
                        sparse_lambda=args.sparse_lambda,
                        sparse_alpha=args.sparse_alpha,
                        mcp_gamma=args.mcp_gamma)
                elif regime == 'annealed_sparse':
                    tr_loss, tr_acc, eff_lambda = train_one_epoch_annealed_sparse(
                        model, train_loader, optimizer, device,
                        sparse_lambda=args.sparse_lambda,
                        sparse_alpha=args.sparse_alpha,
                        epoch=epoch,
                ...
                if epoch % 10 == 0 or epoch == args.epochs:
                    ...
                    extra = (f'  eff_lambda={eff_lambda:.2e}'
                             if regime == 'annealed_sparse' else '')
                    print(f"  Epoch {epoch:3d}: loss={tr_loss:.4f}  "
                          f"train_acc={tr_acc:.4f}  test_acc={te_acc:.4f}{extra}")
    ...
    # Save results
    regime = getattr(args, 'regime', 'standard')
    results_path = os.path.join(args.save_dir, f'bound_results_{regime}.json')
...
def parse_args():
    ...
    parser.add_argument('--plot_path', type=str, default=None,
                        help='Output path for the figure. If omitted, auto-named '
                             'based on --regime.')

    # ── training regime ──────────────────────────────────────────
    parser.add_argument(
        '--regime', type=str, default='standard',
        choices=['standard', 'spectral_decay', 'approx_sparse',
                 'mcp_sparse', 'annealed_sparse'],
        help=(
            'Training regime: '
            'standard (CE + spectral-norm projection), '
            'spectral_decay (CE + spectral-decay penalty + min-sigma floor), '
            'approx_sparse (CE + Cauchy smooth-L0 surrogate), '
            'mcp_sparse (CE + Minimax Concave Penalty; unbiased for large weights), '
            'annealed_sparse (CE + Cauchy surrogate with warmup + linear lambda ramp).'
        ),
    )
    # spectral_decay hyper-parameters
    parser.add_argument('--decay_lambda', type=float, default=1e-3,
                        help='[spectral_decay] penalty weight (default: 1e-3)')
    parser.add_argument('--min_sigma', type=float, default=0.9,
                        help='[spectral_decay] minimum spectral norm floor (default: 0.9)')
    # approx_sparse / annealed_sparse / mcp_sparse shared hyper-parameters
    parser.add_argument('--sparse_lambda', type=float, default=1e-3,
                        help='[*_sparse] penalty weight (default: 1e-3)')
    parser.add_argument('--sparse_alpha', type=float, default=0.1,
                        help='[*_sparse] near-zero threshold alpha (default: 0.1)')
    # mcp_sparse hyper-parameters
    parser.add_argument('--mcp_gamma', type=float, default=3.0,
                        help='[mcp_sparse] MCP truncation parameter gamma; '
                             'penalty saturates at |w| = gamma * alpha (default: 3.0)')
    # annealed_sparse hyper-parameters
    parser.add_argument('--warmup_fraction', type=float, default=0.2,
                        help='[annealed_sparse] fraction of epochs with lambda=0 '
                             '(default: 0.2)')
    parser.add_argument('--anneal_fraction', type=float, default=0.5,
                        help='[annealed_sparse] fraction of epochs over which lambda '
                             'ramps from 0 to sparse_lambda (default: 0.5)')
...
if __name__ == '__main__':
    ...
    # Auto-name the plot file based on regime if no explicit path given
    if args.plot_path is None:
        regime = getattr(args, 'regime', 'standard')
        args.plot_path = f'./figure_D3_{regime}.png'
