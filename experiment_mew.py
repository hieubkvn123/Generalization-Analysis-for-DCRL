...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            'Training regime: '
            'standard (original CE + spectral-norm projection), '
            'spectral_decay (CE + spectral-decay penalty + min-sigma floor), '
            'approx_sparse (CE + smooth L0 surrogate for near-zero weights).'
        ),
    )
    ...
    # approx_sparse hyper-parameters
    parser.add_argument('--sparse_lambda', type=float, default=1e-3,
                        help='[approx_sparse] penalty weight (default: 1e-3)')
    parser.add_argument('--sparse_alpha', type=float, default=0.1,
                        help='[approx_sparse] near-zero threshold alpha (default: 0.1)')
