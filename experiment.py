...
Bounds computed (dominant term only, ignoring constants and log factors):
  ...
  - Ours Thm 3.6             (without loss augmentation)
  - Ours Thm 3.7             (with loss augmentation)
...
def bound_ours_thm36(stats, layer_params, N, B_max, p_ells=None):
    """
...
def bound_ours_thm37(stats, layer_params, N, B_max, B_ells, p_ells=None):
    """
...
def run_experiment(args):
    ...
    for fc_width in args.widths:
        ...
        b_val = bound_ours_thm36(stats, layer_params, N, B_max, p_ells_opt)
        bounds['Ours_Thm36'] = b_val
        print(f"  Ours Thm 3.6 (no loss aug):      {b_val:.4e}")

        b_val = bound_ours_thm37(stats, layer_params, N, B_max, B_ells, p_ells_opt)
        bounds['Ours_Thm37'] = b_val
        print(f"  Ours Thm 3.7 (with loss aug):    {b_val:.4e}")
...
def plot_results(results, save_path='figure_D3.png'):
    ...
    bound_keys = [
        ('Bartlett2017_C1',       'Bartlett et al., 2017\nEq. C1'),
        ('Golowich2018_C6',       'Golowich et al., 2018\nEq. C6'),
        ('Golowich2018_C7',       'Golowich et al., 2018\nEq. C7'),
        ('Neyshabur2018_C5',      'Neyshabur et al., 2018\nEq. C5'),
...
if __name__ == '__main__':
    ...
    print("\nDone. Summary of bounds:")
    print(f"{'Width':>8} | {'Bartlett17':>12} | {'Ours_Thm36':>12} | {'Ours_Thm37':>12}")
    print('-' * 55)
    for w in sorted(results.keys()):
        b = results[w]['bounds']
        print(f"{w:>8d} | {b['Bartlett2017_C1']:>12.4e} | "
              f"{b['Ours_Thm36']:>12.4e} | {b['Ours_Thm37']:>12.4e}")
